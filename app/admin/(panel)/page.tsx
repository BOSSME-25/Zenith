...
import { Users, Heart, Handshake, MessageSquare, Newspaper, Mail, UserPlus } from "lucide-react";
...
const TILES = [
  ...
  { type: "newsletter", label: "Newsletter Issues", icon: Mail },
  { type: "subscribers", label: "Subscribers", icon: UserPlus },
...
async function getCounts() {
  ...
  const [f, c, p, ct, ni, ns, u] = await Promise.all([
    ...
    safeSql<CountRow>`SELECT COUNT(*)::text AS count FROM newsletter_issues`,
  ...
  return [
    ...
    { ...TILES[4], count: Number(ni.rows[0]?.count || 0) },
    { ...TILES[5], count: Number(ns.rows[0]?.count || 0) },
    { ...TILES[6], count: Number(u.rows[0]?.count || 0) },
  ];
}
...
async function getRecent(): Promise<RecentRow[]> {
  ...
  const { rows } = await safeSql<RecentRow>`
    SELECT * FROM (
      ...
      UNION ALL
      SELECT 'subscribers', id, COALESCE(name, 'Subscriber'), email, submitted_at FROM newsletter_subscribers
    ) AS combined
